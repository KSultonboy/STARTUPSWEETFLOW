...
export default function AppLayout() {
    ...
    const hasFeature = (key) => {
        // Hozircha barcha tenantlar uchun to'liq access:
        // agar features umuman kelmasa yoki bo'sh bo'lsa => hamma bo'limlar ochiq.
                    ...
                    <div className="app-sidebar-header">
                        ...
                        {!sidebarCollapsed && (
                            <div className="app-brand">
                                <span className="app-brand-title">{user.tenantName || 'Ruxshona Tort'}</span>
                            ...
                            <>
                                {hasFeature('accounting') && (
                                    <>
                                        <Link className={isActive("/reports")} to="/reports">
                                            ...
                                            {!sidebarCollapsed && <span className="app-nav-label">Hisobotlar</span>}
                                        </Link>
                                ...
                                {hasFeature('sales') && (
                                    <Link className={isActive("/branches")} to="/branches">
                                        <span className="app-nav-icon">🏬</span>
                                        {!sidebarCollapsed && <span className="app-nav-label">Filiallar</span>}
                                    </Link>
                                )}

                                <Link className={isActive("/users")} to="/users">
                                    <span className="app-nav-icon">👥</span>
                                    {!sidebarCollapsed && <span className="app-nav-label">Foydalanuvchilar</span>}
                                </Link>

                                {hasFeature('accounting') && (
                                    <Link className={isActive("/expenses")} to="/expenses">
                                        <span className="app-nav-icon">💸</span>
                                        {!sidebarCollapsed && <span className="app-nav-label">Xarajatlar</span>}
                                    </Link>
                                )}
                                ...
                                {hasFeature('warehouse') && (
                                    <Link className={isActive("/warehouse")} to="/warehouse">
                                        <span className="app-nav-icon">📦</span>
                                        {!sidebarCollapsed && <span className="app-nav-label">Omborxona</span>}
                                    </Link>
                                )}

                                <Link className={isActive("/products")} to="/products">
                                    <span className="app-nav-icon">🍰</span>
                                    {!sidebarCollapsed && <span className="app-nav-label">Asosiy Catalog</span>}
                                </Link>

                                {hasFeature('warehouse') && (
                                    <Link className={isActive("/transfers")} to="/transfers">
                                        <span className="app-nav-icon">🚚</span>
                                        {!sidebarCollapsed && <span className="app-nav-label">Transferlar</span>}
                                    </Link>
                                )}
                                ...
                                {hasFeature('sales') && (
                                    <Link className={isActive("/returns")} to="/returns">
                                        ...
                                        {!sidebarCollapsed && <span className="app-nav-label">Vazvratlar</span>}
                                    </Link>
                                )}

                                <Link className={isActive("/history")} to="/history">
                                    <span className="app-nav-icon">🕒</span>
                                    {!sidebarCollapsed && <span className="app-nav-label">Tarix</span>}
                                </Link>
                            </>
                        )}
                            ...
                            <>
                                ...
                                {hasFeature('sales') && (
                                    <Link className={isActive("/sales")} to="/sales">
                                        <span className="app-nav-icon">💵</span>
                                        {!sidebarCollapsed && <span className="app-nav-label">Sotuv</span>}
                                    </Link>
                                )}
                                {hasFeature('warehouse') && (
                                    <Link className={isActive("/receiving")} to="/receiving">
                                        <span className="app-nav-icon">🚚</span>
                                        {!sidebarCollapsed && <span className="app-nav-label">Qabul qilish</span>}
                                    </Link>
                                )}
                                {hasFeature('sales') && (
                                    <Link className={isActive("/returns")} to="/returns">
                                ...
                                <Link className={isActive("/history")} to="/history">
                                    <span className="app-nav-icon">🕒</span>
                                    {!sidebarCollapsed && <span className="app-nav-label">Sotuv tarixi</span>}
                                </Link>
                            </>
                        )}
                            ...
                            <>
                                ...
                                {hasFeature('production') && (
                                    <Link className={isActive("/production")} to="/production">
                                        <span className="app-nav-icon">🏭</span>
                                        {!sidebarCollapsed && <span className="app-nav-label">Production kiritish</span>}
                                    </Link>
                                )}
                                <Link className={isActive("/history")} to="/history">
                                    <span className="app-nav-icon">🕒</span>
                                    {!sidebarCollapsed && <span className="app-nav-label">Production tarixi</span>}
                                </Link>
                            </>
                        )}
                    </nav>
                            ...
                            <button
                                ...
                                onClick={async () => {
                                    try {
                                        ...
                                        // Fallback: hard redirect to login to avoid blank state
